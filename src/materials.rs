...
impl ColorMaterial {
    pub fn new(gl: &GL) -> Result<Self, String> {
        let program = link(
            gl,
            &compile(&gl, GL::VERTEX_SHADER, COLOR_VERTEX_SHADER)?,
            &compile(&gl, GL::FRAGMENT_SHADER, COLOR_FRAGMENT_SHADER)?,
        )?;
...
impl SingleColorMaterial {
    pub fn new(gl: &GL) -> Result<Self, String> {
        let program = link(
            gl,
            &compile(gl, GL::VERTEX_SHADER, POSITION_3D_VERTEX_SHADER)?,
            &compile(gl, GL::FRAGMENT_SHADER, SINGLE_COLOR_FRAGMENT_SHADER)?,
        )?;
